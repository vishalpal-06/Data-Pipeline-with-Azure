...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(r'..\Log\SmallCap_screener_scraping.log'),
        logging.StreamHandler()  # This will also log to console
    ]
)
logger = logging.getLogger(__name__)

logger.info("Scraping Process Started for SmallCap Companies")
...
file_df.to_csv(r'../Data/Small_Cap.csv', index=False)
logger.info(f"Scraping complete. Data saved to 'Small_Cap.csv'. Total companies scraped: {len(file_df)}")
logger.info("Scraping Process Completed for SmallCap Companies")
# Close the WebDriver
driver.quit()